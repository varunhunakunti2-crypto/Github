    ...
    <div className="flex flex-col gap-lg font-sans text-gray-200">
      ...
      {alerts.length === 0 ? (
        <Card className="bg-[#161B22] border-[#30363D] p-xl rounded-sm text-center">
          ...
          <h3 className="font-sans text-lg font-bold text-white mt-sm">No vulnerable packages</h3>
          ...
          {severitiesOrder.map((severity) => {
            ...
            return (
              <div key={severity} className="flex flex-col gap-sm">
                <h3 className="font-sans font-bold text-sm text-white uppercase tracking-wider flex items-center gap-xs">
                        ...
                        <div>
                          <div className="flex items-center gap-sm">
                            ...
                            <span className={`text-[10px] px-xs font-bold uppercase rounded-full ${
                              alert.status === 'open' ? 'bg-error/20 text-error' : 'bg-gray-700 text-gray-400'
                            }`}>
                              {alert.status}
                            </span>
                          </div>
                            ...
                            {dismissingAlertId === alert.id ? (
                              <div className="flex items-center gap-xs">
                                ...
                                <Button
                                  ...
                                  className="bg-error hover:bg-error/90 text-white text-xs px-xs py-xxs rounded-sm font-semibold"
                                >
                                  Confirm Dismiss
                                </Button>
                                <Button
                              ...
                              <Button
                                ...
                                className="bg-transparent hover:bg-[#282F3B] border border-hairline text-gray-300 text-xs px-sm py-xxs rounded-sm font-semibold"
                              >
                                Dismiss Alert
                              </Button>
                            )}
